...
Require Import CM.Logic.VaryingDomain.
...
Theorem 积极性质可能存在实例 : ⌈∀ Φ, 积极 Φ → 在场一致 Φ⌋.
Proof.
  证明. intros Φ H. 反证.
  apply (在场爆炸原理 w Φ (反 Φ)) in 反设.
...
Definition 实在性 : 性质 := λ x, ∀ Φ, 本性 Φ x → □ ∃ᴱ x, Φ x.

Axiom 实在性积极 : ⌈积极 实在性⌋.

Lemma 实在性可能存在实例 : ⌈在场一致 实在性⌋.
Proof. 证明. apply 积极性质可能存在实例. apply 实在性积极. Qed.

Lemma 可能必然存在反同一性实体 : ⌈◇ □ ∃ᴱ x, 反同一性 x⌋.
Proof.
  证明. eapply 可能性三段论1. apply 实在性可能存在实例.
